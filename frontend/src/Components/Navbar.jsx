...
import {FaRegWindowClose, FaTelegram} from 'react-icons/fa'
import {Link} from 'react-router-dom'
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { FaYoutube, FaFacebook, FaWhatsapp, FaTwitter, FaInstagram, FaLinkedin, FaGithub,} from 'react-icons/fa'
  ...
  return (
    <div ref={cont} className='w-screen fixed shadow-sm shadow-white z-20 bg-gray-800'>
        <div className='flex justify-between items-center py-2 px-4 lg:px-4 ' >
            ...
            <div className='hidden lg:flex lg:flex-row text-white justify-between space-x-[350px] ' >
                <ul className=' flex gap-4 text-[18px] font-bold '>
                   <Link to={'/'}> <li className='hover:text-[#3A0879] cursor-pointer focus:bg- from-gray-800 to-fuchsia-800'>Home</li></Link>
                    <Link to={'/projects'}><li className='hover:text-[#3A0879] cursor-pointer '>Projects</li></Link>
                    <Link to={'/Skills'}> <li className='hover:text-[#3A0879] cursor-pointer'>Skills</li></Link>
                    <Link to={'/Contact'}> <li className='hover:text-[#3A0879] cursor-pointer'>Contact</li></Link>
                </ul>
            ...
            <div className='hidden lg:flex'>
                ...
                <Link to="/Contact"><button className='border-[1px] ml-4 w-[130px] h-[35px] hover:bg-gradient-to-t from-indigo-700 via-fuchsia-600 to-transparent rounded-md text-white font-semibold hover:bg-purple-600 hover:animate-bounce'>Lets Connect</button></Link>
            </div>
        ...
        <div ref={listRef} className='hidden'>
                ...
                <Link to={'/'}> <li className='hover:text-[#3A0879] cursor-pointer hover:border-b-2'>Home</li></Link>
                <Link to={'/projects'}><li className='hover:text-[#3A0879] cursor-pointer'>Projects</li></Link>
                    <Link to={'/Skills'}> <li className='hover:text-[#3A0879] cursor-pointer'>Skills</li></Link>
                    <Link to={'/Contact'}> <li className='hover:text-[#3A0879] cursor-pointer'>Contact</li></Link>
                </ul>
